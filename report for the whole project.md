...
- get inside the hbase using docker: docker exec -it <container_name_or_id> hbase shell
In our case, we use the command: docker exec -it hbase hbase shell
...
# 3. Customer Lifetime Value by City

Customer Lifetime Value (CLV) measures the total revenue generated by customers over time.

Top Cities by Revenue
City	Province	Customers	Revenue (RWF)
Ouahigouya	Yatenga	29	10204.22
Bobo-Dioulasso	Houet	21	6956.94
Ouagadougou	Kadiogo	20	6520.50

# Insight:

-Ouahigouya generated the highest revenue, followed by Bobo-Dioulasso and Ouagadougou. These cities represent important markets for the e-commerce platform.

# 4. Conversion Rate by Device

Device analysis evaluates which devices lead to successful purchases.

Example Device Performance
Device Profile	Sessions	Conversion Rate
Safari Android Desktop	17	64.7%
Firefox Android Desktop	23	56.5%
Safari Android Mobile	29	55.1%

# Insight:

-Mobile devices show strong purchasing activity, suggesting that customers frequently use smartphones to shop online.

# 5. Cohort Analysis

Cohort analysis groups customers based on their registration month to analyze purchasing behavior over time.

Cohort Summary
Registration Month	Users	Total Purchases
2025-12	32	71
2026-01	39	73
2026-02	25	52
...
-The January 2026 cohort contains the largest number of users, while February users have higher average purchase values.

# 6. Conclusion

This project demonstrates how Apache Spark can be used to process and analyze large datasets efficiently.

The analysis identified:

-Product purchase relationships

-Customer revenue patterns by location

-Device conversion performance

-Customer behavior trends over time

These insights can help businesses improve marketing strategies, optimize platforms, and better understand customer behavior.


# Analytics Integration

To demonstrate how multiple big data technologies can work together, an integrated analytical query was implemented using Apache Spark.

# - Business Question

The goal of this analysis is to estimate Customer Lifetime Value (CLV) by combining purchasing activity with user engagement metrics. This helps identify high-value customers who generate the most revenue for the platform.

# - Data Sources

The integrated query combines data from multiple systems:

Data Type	Technology
User profile data	MongoDB
Transaction history	MongoDB
Session activity and browsing behavior	HBase
Analytical processing	Apache Spark

MongoDB stores structured customer and transaction data, while HBase stores high-volume session events such as browsing activity. Spark is used to process and join these datasets efficiently.

# - Processing Workflow

User profile data is loaded from MongoDB.

Transaction records are extracted from completed purchase sessions.

Session activity is analyzed to compute engagement metrics such as session count and average session duration.

Apache Spark joins these datasets using the user_id field.

Customer Lifetime Value is calculated by aggregating revenue generated from purchases.

# -Performance Considerations

Apache Spark performs distributed data processing and allows efficient joins between large datasets. This approach reduces the need for expensive cross-database queries and enables scalable analytical processing.