...
transactions = []
for s in sessions:
    if s.get('converted') and s.get('transaction'):
        t = s['transaction'].copy()
        t['user_id'] = s['user_id']
        t['device'] = s['device']
        t['city'] = s.get('location', 'Unknown')
...
for t in transactions:
    # This is simplified - in real data you'd join with products
    # For now, use random categories from products
    cat = products_df.sample(1)['category'].values[0]
    cat_revenue[cat] = cat_revenue.get(cat, 0) + t['total_rwf']

plt.figure(figsize=(10, 6))
categories = list(cat_revenue.keys())
revenues = list(cat_revenue.values())
...
converted = len([s for s in sessions if s.get('converted')])
...
device_stats = sessions_df.groupby('device').agg({
    'session_id': 'count',
    'converted': 'sum'
}).reset_index()
device_stats.columns = ['device', 'sessions', 'conversions']
device_stats['conversion_rate'] = (device_stats['conversions'] / device_stats['sessions'] * 100).round(1)

plt.figure(figsize=(10, 6))
x = range(len(device_stats['device']))
width = 0.35
...
plt.xticks(x, device_stats['device'])
plt.legend()
